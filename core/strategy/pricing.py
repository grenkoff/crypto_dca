...
up to ``tick_size``. It is also lifted so the sell's notional (``price * qty``)
clears ``min_order_amt`` — otherwise a small (e.g. partially-filled) position
would produce a sub-minimum sell the exchange rejects, leaving the coin naked.
That way the absolute target is used whenever it clears the floors, and the
position is never sold below break-even.
"""
