...
one ``step`` closer to the market so it fills sooner and frees capital. The
step is the grid spacing (``grid_step``), independent of the take-profit
distance.

Target selection: the **second-nearest** TP above the market. The nearest one
is likely to fill on its own; helping the next one down releases locked capital
soonest. (With a single candidate, that one is taken.) Several TPs may end up
stacked on the same price level — that is intended; holes left higher up are
not re-seeded.
...
The re-priced TP always rests as a maker limit: it is floored one tick above
the market (never crosses into a taker fill) and at ``min_order_amt / qty`` so
...
def select_compensation_target(
    ...
    """Pick the position whose TP to pull down: second-nearest above the
    market.

    Only TPs strictly above the market floor are workable. The nearest is left
    to fill naturally; the one behind it gets the step down. With a single
    candidate, take it.
    """
...
def compute_compensation(
    *,
    ...
    if new_tp >= target.current_tp_price:
        # credit/floors don't allow a lower price yet — keep the profit
        return None
