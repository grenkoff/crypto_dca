...
Bar = tuple[float, float, float, float] | None


def test_rescale_ohlc_aligns_first_close() -> None:
    btc: list[Bar] = [
        (100.0, 110.0, 90.0, 100.0),
        (100.0, 130.0, 95.0, 120.0),
    ]
    ref: list[Bar] = [
        (0.02, 0.021, 0.019, 0.025),
...
def test_rescale_ohlc_aligns_at_first_shared_day() -> None:
    btc: list[Bar] = [None, (100.0, 110.0, 90.0, 100.0)]
    ref: list[Bar] = [None, (0.02, 0.021, 0.019, 0.05)]
    out = rescale_ohlc(btc, ref)
...
def test_rescale_ohlc_no_shared_day_returns_unchanged() -> None:
    btc: list[Bar] = [(100.0, 110.0, 90.0, 100.0), None]
    ref: list[Bar] = [None, (0.02, 0.021, 0.019, 0.05)]
