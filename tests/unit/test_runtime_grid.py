...
def test_plan_level_heal_idles_vanished_without_fill() -> None:
    # order 'gone' left the exchange with no fill -> idle the level for
    # re-placement
...
def test_plan_level_heal_replays_vanished_with_fill() -> None:
    # order 'filled' vanished but has a matching fill -> replay to book the
    # position
    fill = _exec("filled")
    awaiting = [(291, "filled")]
...
def test_resting_levels_contiguous_when_nothing_held() -> None:
    ...
    assert Decimal("0.03086") - prices[0] >= Decimal(
        "0.0001"
...
def test_buys_to_prune_cancels_only_below_band_bottom() -> None:
    ...
    assert sorted(buys_to_prune(resting, targets)) == [
        Decimal("0.02930"),
        Decimal("0.02935"),
    ]
...
def test_instance_guard_fresh_heartbeat_blocks() -> None:
    ...
    fresh = now - timedelta(
        seconds=20
    )  # peer beat 20s ago, within the 90s lease
...
def test_instance_guard_stale_heartbeat_allows_start() -> None:
    ...
    stale = now - timedelta(
        seconds=120
    )  # older than the lease ⇒ writer crashed
...
async def test_heal_stale_buy_replay_failure_idles_level_no_loop() -> None:
    ...
    class FailingClient:
        async def get_open_orders(self, symbol: str) -> list[object]:
