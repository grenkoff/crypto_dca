...
class _Book:
    ...
    def _buy_ceiling(self) -> Decimal | None:
        if not self.lots:
            return None
        lowest_tp = min(lot.tp for lot in self.lots)
        return lowest_tp - self.cfg.tp_step - self.cfg.grid_step
    ...
    def fill_tps(self, high: Decimal, price: Decimal) -> None:
        ...
        while True:
            due = [lot for lot in self.lots if lot.tp <= high]
            if not due:
                return
            lot = min(due, key=lambda item: item.tp)
...
def run_backtest(
    ...
    return BacktestResult(
        ...
        usdt=book.usdt,
        base_qty=book.base,
        equity=book.usdt + book.base * last,
